...
def test_init_sets_attributes(rate_limit_manager):
    rl = RateLimit("test_model", 1000, rate_limit_manager)
    assert rl.model_name == "test_model"
...
@patch('logging.info')
def test_cool_down_sleeps_and_resets(mock_info, mock_warning, mock_sleep, rate_limit):
