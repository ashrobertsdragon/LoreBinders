...
mock_after_section.SPACER = "SPACER"
mock_after_section.PAGE_BREAK = "PAGE_BREAK"

MOCK_STYLESHEET = {"Normal": "test", "Heading1": "test1", "Heading2": "test2", "Heading3": "test3", "Title": "test4"}
...
def test_after_section_undefined_member_access():
    with pytest.raises(AttributeError):
        _ = AfterSection.UNDEFINED_MEMBER
...
def test_setup_toc_creates_toc_styles(mock_paragraph_style, mock_toc):
    ...
    mock_paragraph_style.assert_has_calls([
        call("TOCLevel0", parent="test", fontSize=12, leading=14, spaceAfter=6, spaceBefore=6, leftIndent=0),
        call("TOCLevel1", parent="test", fontSize=10, leading=12, spaceAfter=3, spaceBefore=3, leftIndent=10)
    ])
...
def test_add_item_to_story_adds_multiple_flowables(mock_paragraph):
    ...
    assert story == [flowable1, flowable2, mock_after_section.SPACER]
...
def test_add_item_to_story_appends_after_section(mock_paragraph):
    ...
    assert story[-1] == mock_after_section.PAGE_BREAK
...
def test_add_item_to_story_handles_empty_story_list(mock_paragraph):
    ...
    assert story == [flowable, mock_after_section.SPACER]
...
def test_add_item_to_story_handles_empty_flowables_argument(mock_paragraph):
    ...
    assert story == [mock_after_section.PAGE_BREAK]
    mock_paragraph.assert_not_called()
...
def test_add_item_to_story_handles_none_story_list(mock_paragraph):
    story = None
    with pytest.raises(AttributeError):
        add_item_to_story(story, mock_after_section.SPACER)
    mock_paragraph.assert_not_called()
...
def test_add_item_to_story_handles_existing_story_list(mock_paragraph):
    ...
    assert story == [flowable1, flowable2, mock_after_section.PAGE_BREAK]
...
def test_add_content_adds_trait(mock_add_item_to_story, mock_create_detail_list, mock_list_flowable, mock_paragraph):
    story = []
    style = MOCK_STYLESHEET
    content = {"trait": {"1": "Test Trait"}}

    add_content(story, content, style, is_traits=True)

    mock_paragraph.assert_called_once_with("trait", MOCK_STYLESHEET["Heading3"])
...
def test_add_content_adds_multiple_traits(mock_add_item_to_story, mock_create_detail_list, mock_list_flowable, mock_paragraph):
    ...
    mock_paragraph.assert_has_calls([call("trait1", MOCK_STYLESHEET["Heading3"]), call("trait2", MOCK_STYLESHEET["Heading3"]), call("trait3", MOCK_STYLESHEET["Heading3"])])
...
def test_add_content_adds_content_multiple_chapters_multiple_traits_is_traits_true(mock_add_item_to_story, mock_create_detail_list, mock_list_flowable, mock_paragraph):
    ...
    mock_paragraph.assert_has_calls([call("trait", MOCK_STYLESHEET["Heading3"]), call("trait2", MOCK_STYLESHEET["Heading3"])])
...
def test_initialize_pdf_creates_pdf_in_user_folder(mock_simple_doc_template, mock_path):
    ...
    initialize_pdf(metadata)
    mock_simple_doc_template.assert_called_once_with(
        mock_path("specified_folder", "Test Title-lorebinder.pdf"),
        pagesize=LETTER,
        author="Test Author",
        title="Test Title"
    )
...
@patch("lorebinders.make_pdf.AfterSection", mock_after_section)
@patch("lorebinders.make_pdf.getSampleStyleSheet", return_value=MOCK_STYLESHEET)
...
def test_create_pdf_successfully_creates_pdf(mock_initialize_pdf, mock_setup_toc, mock_create_paragraph, mock_add_item_to_story, mock_add_content, mock_add_image, mock_get_sample_stylesheet, mock_cast):
    binder = {"Characters": {"John Doe": {"summary": "A brave hero"}}}
    book = Mock()
    book.binder = binder
    book.metadata = Mock()
    book.metadata.return_value.title = "Test Book"
    mock_doc = Mock()
    mock_doc.page = 1
...
@patch("lorebinders.make_pdf.setup_toc")
@patch("lorebinders.make_pdf.initialize_pdf")
def test_create_pdf_adds_title_page_and_toc(mock_initialize_pdf, mock_setup_toc, mock_create_paragraph, mock_add_item_to_story, mock_add_content, mock_add_image, mock_get_sample_stylesheet, mock_cast, binder):
    book = Mock()
    book.binder = binder
    book.metadata = Mock()
    book.metadata.return_value.title = "Test Book"
    ...
    mock_initialize_pdf.return_value = (mock_doc, "Test Book")
    mock_create_paragraph.return_value = Mock()
    mock_cast.side_effect = ["path/to/image1.jpg", "Summary for Character1", "path/to/image2.jpg", "Summary for Setting1"]
...
@patch("lorebinders.make_pdf.setup_toc")
@patch("lorebinders.make_pdf.initialize_pdf")
def test_create_pdf_handles_empty_binder(mock_initialize_pdf, mock_setup_toc, mock_create_paragraph, mock_add_item_to_story,  mock_add_content, mock_add_image, mock_get_sample_stylesheet):
    book = Mock()
    book.binder = {}
    book.metadata = Mock()
    book.metadata.return_value.title = "Test Book"
    ...
    mock_initialize_pdf.assert_called_once_with(book.metadata)
    mock_setup_toc.assert_called_once()
    mock_create_paragraph.assert_called_once()
    mock_create_paragraph.assert_any_call("LoreBinder\nfor\nTest Book", mock_get_sample_stylesheet.return_value["Title"])
    mock_add_item_to_story.assert_any_call(ANY, mock_after_section.PAGE_BREAK, ANY)
...
@patch("lorebinders.make_pdf.setup_toc")
@patch("lorebinders.make_pdf.initialize_pdf")
def test_create_pdf_handles_missing_summary_or_content_fields(mock_initialize_pdf, mock_setup_toc, mock_create_paragraph, mock_add_item_to_story, mock_add_content, mock_add_image, mock_get_sample_stylesheet):
    ...
    mock_create_paragraph.assert_has_calls([
        call("LoreBinder\nfor\nTest Book", mock_get_sample_stylesheet.return_value["Title"]),
        call("Characters", mock_get_sample_stylesheet.return_value["Heading1"]),
        call("John Doe", mock_get_sample_stylesheet.return_value["Heading2"])
    ])
    assert mock_add_item_to_story.call_count == 4
    mock_add_content.assert_called_once()
    mock_add_image.assert_not_called()
    mock_add_content.assert_called_once()
    mock_multiBuild.assert_called_once()
